...
namespace ILCalc
{
  ...
  public static class Interpret
  {
    ...
    public static Array Allocate<T>(params ValueRange<T>[] ranges)
    {
      ...
      if (ranges.Length == 1)
        return new T[ranges[0].ValidCount];
      ...
      return AllocImpl<T>(0, sizes);
    }

    #endregion
    #region Internals

    private static Array AllocImpl<T>(int pos, params int[] sizes)
    {
      if (sizes.Length - pos == 2)
      {
        var array = new T[sizes[pos]][];
      ...
      else
      {
        var array = Array.CreateInstance(
          TypeHelper<T>.GetArrayType(sizes.Length - pos - 1),
          sizes[pos++]);

        for (int i = 0; i < array.Length; i++)
        {
          array.SetValue(AllocImpl<T>(pos, sizes), i);
        }
